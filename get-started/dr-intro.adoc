...
summary: Die Notfallwiederherstellung in der Cloud ist eine robuste und kostengünstige Methode, um Workloads vor Standortausfällen und Datenbeschädigung zu schützen. Mit NetApp Disaster Recovery können Sie Ihre lokalen VMware-Workloads, die auf ONTAP-Speicher laufen, in ein softwaredefiniertes VMware-Rechenzentrum in einer öffentlichen Cloud mithilfe von NetApp Cloud-Speicher oder in eine andere lokale VMware-Umgebung mit ONTAP-Speicher als Disaster-Recovery-Standort replizieren. 
---
...
[role="lead"]
Die Notfallwiederherstellung in der Cloud ist eine resiliente und kostengünstige Methode, um Workloads vor Standortausfällen und Datenbeschädigung zu schützen. NetApp Disaster Recovery ist ein Cloud-basierter Disaster-Recovery-Service, der Disaster-Recovery-Workflows automatisiert. Mit NetApp Disaster Recovery können Sie Ihre lokalen Kubernetes-Cluster, NFS-basierte Workloads und VMware vSphere Virtual Machine File System (VMFS)-Datenspeicher für iSCSI- und FC-basierte NetApp Speichersysteme auf eine der folgenden Arten schützen:

* Amazon Elastic VMware Service (EVS) mit Amazon FSx für NetApp ONTAP. Weitere Informationen unter link:../reference/evs-deploy-guide-introduction.html["Einführung in die NetApp Disaster Recovery mit Amazon Elastic VMware Service und Amazon FSx für NetApp ONTAP"].
* VMware Cloud (VMC) auf AWS mit Amazon FSx for NetApp ONTAP
* Azure VMware Solution mit NetApp Cloud Volumes ONTAP (iSCSI) (Private Preview)
...
image:../use/evs-console-architecture.png["Diagramm zur Darstellung der Komponenten der NetApp Disaster Recovery"]
...
== Vorteile der Nutzung von NetApp Disaster Recovery
...
== Fähigkeiten von NetApp Disaster Recovery
...
* Führen Sie einen Failover-Test durch. Dabei erstellt der Dienst temporäre virtuelle Maschinen. Disaster Recovery erstellt ein neues FlexClone-Volume aus dem ausgewählten Snapshot, und ein temporärer Datenspeicher, der durch das FlexClone-Volume unterstützt wird, wird den ESXi-Hosts zugeordnet. Dieser Prozess beansprucht keine zusätzliche physische Kapazität auf lokalem ONTAP-Speicher oder FSx für NetApp ONTAP-Speicher in AWS. Das ursprüngliche Quellvolume wird nicht verändert und Replikationsaufträge können auch während der Disaster Recovery fortgesetzt werden.
...
image:../use/evs-use-cases.png["Diagramm mit Anwendungsfällen für NetApp Disaster Recovery"]


NOTE: Sie müssen den vSphere Server außerhalb von NetApp Disaster Recovery in vSphere Server konfigurieren.
...
* Ein Pay-as-you-go (PAYGO)-Abonnement kann über den Amazon Web Services (AWS) Marketplace, den Azure Marketplace oder den Google Cloud Marketplace erworben werden. Diese Lizenz ermöglicht den Erwerb einer Lizenz mit fester, geschützter Kapazität ohne langfristige Verpflichtung.
...
Die Lizenzierung der Notfallwiederherstellung basiert auf der Datenmenge, die auf geschützten ONTAP Volumes gespeichert ist. Der Dienst ermittelt die für die Lizenzierung relevanten Volumes durch die Zuordnung geschützter VMs zu ihren vCenter Datenspeichern. Jeder Datenspeicher befindet sich auf einem ONTAP Volume oder einer LUN. Die von ONTAP für dieses Volume oder diese LUN gemeldete belegte Kapazität dient der Lizenzbestimmung.
...
NOTE: NetApp Disaster Recovery-Gebühren basieren auf der genutzten Kapazität der Datenspeicher am Quellstandort, wenn mindestens eine VM oder Kubernetes-Ressource über einen Replikationsplan verfügt. Die Kapazität eines übergewechselten Datenspeichers ist nicht im Kapazitätskontingent enthalten. Bei einer BYOL-Lizenz sind die Vorgänge im Dienst eingeschränkt, wenn die Datenmenge die zulässige Kapazität überschreitet, bis eine zusätzliche Kapazitätslizenz erworben oder die Lizenz in der NetApp Console aktualisiert wird.
...
Sie können NetApp Disaster Recovery 30 Tage lang kostenlos testen.

Um nach der 30-tägigen Testphase fortfahren zu können, benötigen Sie ein Pay-as-you-go (PAYGO)-Abonnement von Ihrem Cloud-Anbieter oder müssen eine BYOL-Lizenz von NetApp erwerben.
...
image:dr-architecture-diagram-70-2.png["Diagramm zur Darstellung der Architektur der NetApp Disaster Recovery Service-Infrastruktur"]
...
Für die Notfallwiederherstellung werden folgende Komponenten verwendet:
...
* VMware Cloud (VMC) auf AWS mit Amazon FSx for NetApp ONTAP
* Eine weitere lokale, NFS-basierte VMware-Umgebung mit ONTAP Storage oder eine lokale FC/iSCSI VMFS-Umgebung
