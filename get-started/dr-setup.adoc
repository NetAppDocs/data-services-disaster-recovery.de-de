...
[role="lead"]
Um NetApp Disaster Recovery zu nutzen, führen Sie einige Schritte durch, um es sowohl in der NetApp Console als auch in der unterstützten Notfallwiederherstellungsumgebung einzurichten.
...
* *AWS Site-to-Site-VPN*: Diese virtuelle private Netzwerkverbindung kann verwendet werden, um sichere Zugriffsverbindungen zusammen mit einem öffentlichen Internetdienstanbieter bereitzustellen. Das VPN verschlüsselt und entschlüsselt alle Daten, die zur und von der AWS Cloud übertragen werden. VPNs können entweder software- oder hardwarebasiert sein. Für Unternehmensanwendungen sollte der öffentliche Internetdienstanbieter (ISP) Quality-of-Service-Garantien bieten, um sicherzustellen, dass für die DR-Replikation eine ausreichende Bandbreite und Latenz bereitgestellt werden.
