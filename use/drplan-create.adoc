...
* *Originale Ordnerhierarchie beibehalten*: Wenn diese Option ausgewählt ist, behält Disaster Recovery die VM-Inventarhierarchie (Ordnerstruktur) beim Failover bei. Falls das Wiederherstellungsziel _nicht_ die ursprüngliche Ordnerhierarchie aufweist, erstellt Disaster Recovery diese. Aktivieren Sie das Kontrollkästchen, um die Ordnerhierarchie beizubehalten; deaktivieren Sie es, um die ursprüngliche Ordnerhierarchie zu ignorieren.
