...
[role="lead"]
Nachdem Sie Ressourcen und Standorte hinzugefügt haben, können Sie einen _Replikationsplan_ erstellen, um den Datenschutz zu verwalten. Das Erstellen des Replikationsplans umfasst das Festlegen der Quell- und Zielstandorte, die Auswahl der Ressourcengruppen sowie die Auswahl, wie Anwendungen wiederhergestellt und eingeschaltet werden sollen. Beispielsweise können Sie virtuelle Maschinen (VMs) gruppieren, die einer Anwendung zugeordnet sind, oder Sie können Anwendungen mit ähnlichen Tiers gruppieren.

Beim Erstellen eines Replikationsplans besteht außerdem die Möglichkeit, Zeitpläne für Compliance und Tests zu bearbeiten und Test-Failover durchzuführen, ohne die Produktionsdaten zu beeinträchtigen.
...
* Speichern Sie geschützte und ungeschützte Ressourcen nicht auf demselben Datenspeicher. Wenn geschützte und ungeschützte Ressourcen auf demselben Datenspeicher gespeichert werden, können folgende Probleme auftreten:
+
...
** Wenn die Ressourcengruppe und die zugehörigen Ressourcen auf den Notfallwiederherstellungsstandort verschoben werden müssen, sind nach dem Failover-Prozess keine ungeschützten Ressourcen mehr am Quellstandort vorhanden, was zum Ausfall ungeschützter Ressourcen am Quellstandort führt. Außerdem wird NetApp Disaster Recovery diese ungeschützten Ressourcen am Failover-Standort nicht starten.
...
+
** Um eine VM oder einen Datenspeicher zu entfernen, bewegen Sie den Mauszeiger über den Namen der Datenquelle und wählen Sie dann *X*.
** Um die Ressourcen in mehrere Ressourcengruppen zu organisieren, wählen Sie *Click to add another resource group*. Nachdem Sie die Ressourcengruppe hinzugefügt haben, können Sie Ressourcen per Drag & Drop zwischen den Gruppen verschieben. Wählen Sie das Stiftsymbol*, um den Namen der Ressourcengruppe zu bearbeiten.
+
...
NOTE: Aktivieren Sie die erweiterten Einstellungen, um *Skripte* und *applikationskonsistente* Einstellungen hinzuzufügen.
