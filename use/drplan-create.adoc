...
. Wählen Sie im Menü NetApp Disaster Recovery die Option *Replikationspläne* und anschließend *Hinzufügen*. Wenn Sie zum ersten Mal einen Replikationsplan erstellen, wählen Sie im Dashboard *Replikationsplan hinzufügen*.
. Geben Sie einen Namen für den Replikationsplan ein.
. Wählen Sie unter *Quelle vCenter* das vCenter aus, in dem sich die Daten befinden, im Dropdown-Menü. Wählen Sie unter *Ziel vCenter* das vCenter aus, in das die Daten repliziert werden sollen, im Dropdown-Menü.
+
...
* Hosten Sie geschützte und ungeschützte VMs nicht auf demselben Datenspeicher. Wenn geschützte und ungeschützte VMs auf demselben Datenspeicher gehostet werden, können diese Probleme auftreten:
+
** Die belegte Kapazität dieses Volumes kann in die Lizenzberechnung einfließen, weil Disaster Recovery SnapMirror verwendet, was bedeutet, dass das System ganze ONTAP Volumes repliziert. In diesem Fall würde der von geschützten und ungeschützten VMs belegte Speicherplatz in diese Berechnung einbezogen.
...
* *CPU* und *Arbeitsspeicher (GB)*: Unter den Details der virtuellen Maschinen können Sie optional die VM-CPU und den Arbeitsspeicher anpassen.
+
