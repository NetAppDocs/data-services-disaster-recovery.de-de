...
[role="lead"]
Nachdem Sie Ressourcen und Standorte hinzugefügt haben, wird ein Replikationsplan erstellt, um Ihre Daten zu schützen. Der Plan definiert die Quell- und Zielstandorte, Ressourcengruppen, Wiederherstellungszuordnungen und das Einschaltverhalten Ihrer Anwendungen. Es besteht außerdem die Möglichkeit, Compliance- und Testzeitpläne zu konfigurieren und Test-Failover durchzuführen, ohne die Produktionsdaten zu beeinträchtigen.
...
Der Schutz kann nur aktiviert werden, wenn sich der Replikationsplan in einem der folgenden Zustände befindet:
...
=== Snapshots des Replikationsplans
...
** Falls auf der Zielseite Snapshots fehlen, kann der Dienst die entsprechenden Snapshots auf der Quellseite während des nächsten geplanten Snapshot-Abgleichs löschen, abhängig von der Standard-SnapMirror-Richtlinie für die Beziehung.
...
Die Notfallwiederherstellung führt alle 24 Stunden Snapshot-Symmetrieprüfungen durch.



[[considerations-for-platform-managed-backups]]
=== Überlegungen zu plattformverwalteten Backups

Wenn Sie plattformverwaltete Backups (das heißt, ONTAP-verwaltete Snapshots) verwenden, wird die Verwendung der *MirrorAll* Replikationsrichtlinie empfohlen. Die Tabelle zeigt die Auswirkungen der verschiedenen SnapMirror Replikationsrichtlinien und erläutert, warum MirrorAll die empfohlene Option ist.

[cols="10,10,80"]
|===


| Richtlinie | Empfehlung | Hinweise 


| *MirrorAll* | Empfohlen | Keine SnapMirror Transfer-Snapshots für Failover- oder Failback-Vorgänge verwenden. 


| *MirrorVault* .2+| Alternative Empfehlung .2+| Stellen Sie sicher, dass die während des Failovers oder Failbacks ausgewählten Snapshots sowohl auf dem Quell- als auch auf dem Zielvolume vorhanden sind. Andernfalls tritt der Fehler auf: „Übergang in den Zustand 'snapmirrored' fehlgeschlagen. Grund: Kein gemeinsamer Snapshot gefunden.“ Keine SnapMirror Transfer-Snapshots für Failover- oder Failback-Vorgänge verwenden. 


| *Asynchron* 


| *MirrorLatest* | NICHT empfohlen | Diese Vorgehensweise führt dazu, dass das Zielsystem effektiv nur einen gemeinsamen Snapshot für Failover besitzt, was für Disaster Recovery-Szenarien suboptimal ist. 
|===


[[considerations-for-disaster-recovery-managed-backups]]
=== Überlegungen zu von Disaster Recovery verwalteten Backups

Bei der Verwendung von Disaster Recovery verwalteten Backups gelten die folgenden Richtlinien für SnapMirror Beziehungen, die außerhalb von NetApp Disaster Recovery erstellt wurden:

* SnapMirror Beziehungen können die MirrorAll, MirrorVault, MirrorLatest und asynchronen Replikationsrichtlinien unterstützen.
* Planen Sie *keine* Aktualisierungen für diese Beziehungen ein, da diese vom Disaster Recovery Service verwaltet werden.
...
* Wenn Sie Netzwerkzuordnungen auf der vCenter-Ebene konfigurieren, erkennt Disaster Recovery diese Zuordnungen beim Erstellen des Replikationsplans. Wenn Sie die Netzwerkzuordnung auf der vCenter-Ebene nach dem Erstellen des Replikationsplans ändern oder hinzufügen, erkennt Disaster Recovery diese Zuordnungen bei der manuellen Bearbeitung des Replikationsplans.
* Mit Google Cloud können Sie einem Replikationsplan nur ein Volume oder einen Datenspeicher hinzufügen.



TIP: Organisieren Sie Ihre VMs oder Kubernetes-Cluster, bevor Sie NetApp Disaster Recovery bereitstellen, um eine unkontrollierte Ausbreitung zu minimieren. Platzieren Sie Ressourcen, die geschützt werden müssen, auf einer Teilmenge der Datenspeicher und Ressourcen, die nicht geschützt werden sollen, auf einer anderen Teilmenge der Datenspeicher. Um Ressourcengruppen vor der Erstellung des Replikationsplans zu organisieren, siehe link:resource-group.html["Erstellen einer Ressourcengruppe"].



[[create-the-replication-plan]]
== Erstellen des Replikationsplans

Der Assistent führt Sie durch diese Schritte:

* Wählen Sie vCenter-Server aus.
* Wählen Sie die Ressourcen (VMs, Datenspeicher, Namespaces) aus, die Sie replizieren möchten, und weisen Sie Ressourcengruppen zu.
...
Befolgen Sie beim Erstellen des Plans diese Richtlinien:

* Verwenden Sie für alle VMs oder Kubernetes-Cluster im Plan dieselben Anmeldeinformationen.
* Verwenden Sie dasselbe Skript für alle VMs oder Kubernetes-Cluster im Plan.
* Verwenden Sie für alle VMs oder Kubernetes-Cluster im Plan dasselbe Subnetz, DNS und Gateway.
...
. Wählen Sie im Dropdown-Menü den Ressourcentyp aus, für den Sie einen Replikationsplan erstellen möchten: *Kubernetes* oder *vCenter*.
+
image::site-type-toggle.png[Screenshot der Umschaltung des Website-Typs]

. Wählen Sie *Hinzufügen*.
. Geben Sie einen Namen für den Replikationsplan ein.
. Wählen Sie unter *Quelle* den vCenter oder Kubernetes-Cluster aus, der als Quelle festgelegt werden soll.
. Wählen Sie unter *Quelle vCenter* im Dropdown-Menü das vCenter aus, in dem sich die Daten befinden. Wählen Sie unter *Ziel* das vCenter oder den Kubernetes-Cluster aus, der als Ziel für die Notfallwiederherstellung verwendet werden soll.
+
image:dr-plan-create-name2.png["Screenshot, der die vCenter-Serverseite zeigt"]
...
Im nächsten Schritt werden die benötigten VMs, Datenspeicher oder Kubernetes-Cluster in funktionale Ressourcengruppen zusammengefasst. Ressourcengruppen ermöglichen es, eine Gruppe von Ressourcen wie einen Kubernetes-Cluster oder eine VM mit einem gemeinsamen Snapshot zu schützen.

Berücksichtigen Sie beim Erstellen von Ressourcengruppen die folgenden Punkte:

* Bevor Sie Ressourcen zu Ressourcengruppen hinzufügen, starten Sie zunächst eine manuelle oder geplante Ermittlung. Dadurch wird sichergestellt, dass die Ressourcen ermittelt und in der Ressourcengruppe aufgelistet werden.
* Stellen Sie sicher, dass sich mindestens eine VM im Datenspeicher befindet. Wenn sich keine VMs im Datenspeicher befinden, wird der Datenspeicher nicht erkannt.
* Ein einzelner Datenspeicher sollte keine VMs hosten, die durch mehr als einen Replikationsplan geschützt sind.
* Geschützte und ungeschützte Ressourcen sollten nicht auf demselben Datenspeicher gehostet werden. Wenn geschützte und ungeschützte Ressourcen auf demselben Datenspeicher gehostet werden, können folgende Probleme auftreten:
+
** Die belegte Kapazität dieses Volumes kann in die Lizenzberechnung einfließen, weil Disaster Recovery SnapMirror verwendet, was bedeutet, dass das System gesamte ONTAP Volumes repliziert. In diesem Fall würde der von geschützten und ungeschützten Ressourcen belegte Speicherplatz in diese Berechnung einbezogen.
** Wenn die Ressourcengruppe und die zugehörigen Ressourcen auf den Notfallwiederherstellungsstandort überführt werden müssen, existieren ungeschützte Ressourcen nach dem Failover-Prozess am Quell-Standort nicht mehr, was zum Ausfall ungeschützter Ressourcen am Quell-Standort führt. Außerdem wird NetApp Disaster Recovery diese ungeschützten Ressourcen am Failover-Standort nicht starten.





TIP: Erstellen Sie für Ihre Failover-Tests einen separaten, dedizierten Satz von Zuordnungen, um zu verhindern, dass Ressourcen mit Produktionsnetzwerken über dieselben IP-Adressen verbunden werden.
...
--
.Schritte
. Wenn Sie bereits Ressourcengruppen haben, wählen Sie **Ressourcengruppen**, wählen Sie dann die Ressourcengruppe und anschließend **Weiter**.
+
Wenn Sie keine vorhandenen Ressourcengruppen haben oder Ressourcen zu einer hinzufügen müssen, wählen Sie *Virtual machines* oder *Datastores*.

. Wählen Sie die virtuellen Maschinen oder Datenspeicher aus der automatisch generierten Liste aus, die Sie dem Replikationsplan hinzufügen möchten. Für virtuelle Maschinen können Sie nach dem Datenspeicher filtern. Wenn Sie einen Datenspeicher oder eine VM auswählen, wird sie automatisch einer Ressourcengruppe hinzugefügt.
+
image:dr-plan-create-apps-vms6.png["Screenshot der Seite „Replikationsplan hinzufügen > Zu replizierende Anwendungen“"]

. Überprüfen Sie auf der rechten Hälfte der Seite die ausgewählten VMs oder Datenspeicher.
+
** Um eine VM oder einen Datenspeicher zu entfernen, den Mauszeiger über den Namen der Ressource bewegen und anschließend *X* auswählen.
** Um die Ressourcen in mehrere Ressourcengruppen zu organisieren, *Weitere Ressourcengruppe hinzufügen* auswählen. Nach dem Hinzufügen der Ressourcengruppe können Ressourcen per Drag & Drop zwischen den Gruppen verschoben werden. Das Stiftsymbol auswählen, um den Namen der Ressourcengruppe zu bearbeiten.
+
...
.Kubernetes
--
Beim Erstellen eines Replikationsplans für Kubernetes-Cluster müssen die Kubernetes-Namespaces in einer Ressourcengruppe zum selben ONTAP Cluster gehören.

.Schritte
. Falls Sie bereits Ressourcengruppen haben, wählen Sie die Gruppen aus der Liste aus.
+
Wenn Sie keine vorhandenen Ressourcengruppen haben oder Ressourcen zu einer solchen hinzufügen müssen, wählen Sie *+ Neue Ressourcengruppe erstellen*. Folgen Sie den Anweisungen link:resource-group.html["Füge einer Ressourcengruppe Namensräume hinzu"].

+
Bei der Benennung einer Ressourcengruppe sind nur Kleinbuchstaben und Zahlen als unterstützte Zeichen zulässig.

. Überprüfen Sie in der alternativen Spalte die ausgewählten Ressourcengruppen.
. Wählen Sie **Weiter**.
...
--
.Schritte
. Wählen Sie die Namespace-Zuordnungen für jeden Namespace in jeder Ressourcengruppe. Standardmäßig wählt NetApp Disaster Recovery das Ziel so aus, dass es mit der Quelle identisch ist. Sie können im Ziel-Cluster einen anderen Namespace auswählen.
. Weisen Sie jeder Ressourcengruppe die Speicherklassenzuordnung zu. Die Speicherklasse muss mit einer ONTAP Storage-VM verknüpft sein, die mit der Quellumgebung gepaart ist. Die Standard-Speicherklasse des Clusters wird verwendet, wenn Sie keine zuweisen.
+

TIP: Führen Sie den CLI-Befehl  `kubectl get pvc -n <namespace>` im Namespace aus, um die Speicherklasse anzuzeigen.

. Snapshot-Zeitplan festlegen:
+
Wählen Sie den *Startzeitpunkt für Snapshots und Aufbewahrung* (Kalenderdatum und Uhrzeit), die *Snapshot- und Aufbewahrungshäufigkeit* (wie oft Snapshots initiiert werden) und die *Snapshot-Aufbewahrungsanzahl* (wie viele Snapshots gespeichert werden).

+
Bei der Festlegung der Snapshot-Frequenz muss eine Frequenz von mindestens fünf Minuten gewählt werden.

. Wählen Sie *Hinzufügen AppVault*, um den AppVault zu erstellen, in dem die Schutzdaten gespeichert werden.
+
...
+
Für Azure sind das *Abonnement*, die *Ressourcengruppe* und das *Speicherkonto* für den Cloud-Anbieter auszuwählen.

+
Für AWS, ONTAP S3 und StorageGRID das *Konto*, die *Anmeldeinformationen* und den *Bucket* auswählen.
...
.. Wählen Sie **Hinzufügen**.
+
image:kubernetes-mapping.png["Screenshot der Storage-Klassenzuordnung für Kubernetes"]



. Wählen Sie **Weiter**.
. Überprüfen Sie die Failover-Zuordnungen und wählen Sie dann **Hinzufügen**, um den Replikationsplan zu erstellen.
+
Nach Auswahl von „Hinzufügen“ können Sie die verschiedenen Jobs in **Job monitoring** überwachen.
...
==== Ressourcentransformationen und Skriptausführungen

Für Kubernetes-Replikationspläne können Ressourcentransformationen und Skriptausführungen angewendet werden. Diese Schritte sind optional.

.Eine Ressourcentransformation hinzufügen
Durch das Hinzufügen einer Ressourcentransformation wird es möglich, jeden Aspekt einer Arbeitslast während eines Failovers zu ändern. Ressourcentransformationen bieten Flexibilität, um Arbeitslasten während eines Notfallwiederherstellungsereignisses an unterschiedliche Umgebungen und Konfigurationen anzupassen. link:task-resource-transformation.html["Informationen zur Erstellung und Ressourcenzuweisung von Transformationen."^]

Ressourcentransformationen werden in der Reihenfolge angewendet, in der sie aufgelistet sind.

. Wählen Sie im Dropdown-Menü die Ressourcentransformation aus, die hinzugefügt werden soll.
. Wählen Sie aus, auf welche Ressourcen die Transformation angewendet werden soll, und fügen Sie die erforderlichen Werte hinzu.
. Die Reihenfolge der Transformationen sollte überprüft werden, damit die Operationen in der richtigen Reihenfolge ausgeführt werden.


.Eine Skriptausführung hinzufügen
NetApp Disaster Recovery kann vor oder nach der Erstellung eines Snapshots ein Skript ausführen, um den Ruhezustand und die Wiederaufnahme einer Anwendung zu steuern. Durch diese Vorgehensweise ist gewährleistet, dass Anwendungen vor der Replikation und Wiederherstellung eines Snapshots ordnungsgemäß vorbereitet sind und nach einem Failover nahtlos weiterarbeiten können.

. *+ Skript hinzufügen* auswählen.
. *Hochladen* auswählen, um die Skriptdatei auszuwählen.
. Wählen Sie die *Ausführungsphase*: *Vor dem Snapshot*, um die Ausführung vor der Erstellung des Snapshots auszuführen, oder *Nach dem Snapshot*, um die Ausführung danach auszuführen.
. Wählen Sie *Speichern*.




==== Abschnitt Compute-Ressourcen

Im Abschnitt „Compute-Ressourcen“ wird festgelegt, wo VMs nach einem Failover wiederhergestellt werden. Das Quell-vCenter-Rechenzentrum und der Quell-Cluster werden einem Ziel-Rechenzentrum und Ziel-Cluster zugeordnet.

Optional können VMs auf einem bestimmten vCenter ESXi-Host neu gestartet werden. Wenn VMware DRS aktiviert ist, kann die VM bei Bedarf automatisch auf einen alternativen Host verschoben werden, um die konfigurierte DR-Richtlinie zu erfüllen.

Optional können alle VMs dieses Replikationsplans in einem eigenen Ordner innerhalb des vCenter-Inventars abgelegt werden. Dies bietet eine einfache Möglichkeit, fehlübernommene VMs im vCenter schnell zu organisieren.
...
VMs verwenden virtuelle Netzwerkkarten, die mit virtuellen Netzwerken verbunden sind. Im Failover-Prozess verbindet der Dienst diese virtuellen Netzwerkkarten mit virtuellen Netzwerken, die in der Ziel-VMware-Umgebung definiert sind. Für jedes von den VMs in der Ressourcengruppe verwendete Quellnetzwerk benötigt der Dienst eine Zuordnung zu einem Zielnetzwerk.


NOTE: Dieser Abschnitt ist nur für vCenters erforderlich. Sie müssen ihn für Kubernetes nicht ausfüllen.
...
Im Tab „Failover-Zuordnungen“ den Abwärtspfeil neben *Virtuelle Netzwerke* auswählen. Das virtuelle Quellnetzwerk und das virtuelle Zielnetzwerk auswählen.
...
NOTE: *Erweiterte Einstellungen* aktivieren, um *Skripte* und *Applikationskonsistenz* zu konfigurieren.

*Netzwerkeinstellungen* NetApp Disaster Recovery unterstützt eine umfangreiche Auswahl an Konfigurationsoptionen für VM-Netzwerke. Eine Änderung dieser Optionen kann erforderlich sein, wenn der Zielstandort virtuelle Netzwerke mit anderen TCP/IP-Einstellungen als die Produktionsnetzwerke am Quellstandort verwendet.
...
* *IP-Adresstyp*: Die VM-Konfiguration ist so anzupassen, dass sie den Anforderungen des Ziel-Clusters im virtuellen Netzwerk entspricht. NetApp Disaster Recovery bietet zwei Optionen: DHCP oder statische IP. Für statische IPs sind die Subnetzmaske, das Gateway und die DNS-Server zu konfigurieren. Zusätzlich sind Anmeldeinformationen für die VMs einzugeben.
+
...
+
*VMs mit Microsoft Windows*: können entweder Windows-Batch (.bat) oder PowerShell (ps1) Skripte ausführen. Windows-Skripte können Befehlszeilenargumente verwenden. Jedes Argument ist im  `arg_name$value` Format zu schreiben, wobei  `arg_name` der Name des Arguments und  `$value` der Wert des Arguments ist, und jedes  `argument$value` Paar wird durch ein Semikolon getrennt.
...
* *CPU* und *Speicher (GB)*: Unter den Details der virtuellen Maschinen können Sie optional die CPU und den Speicher der VM anpassen.
+
...
+
** Sequenzieller Start: Jeder VM wird eine eindeutige Nummer zugewiesen, sodass sie in der festgelegten Reihenfolge gestartet werden, zum Beispiel 1,2,3,4,5.
...
* *Applikationskonsistent*: Geben Sie an, ob applikationskonsistente Snapshot-Kopien erstellt werden sollen. Der Service wird die Anwendung anhalten und dann einen Snapshot erstellen, um einen konsistenten Zustand der Anwendung zu erhalten. Diese Funktion wird mit Oracle unter Windows und Linux sowie mit SQL Server unter Windows unterstützt. Weitere Details folgen als Nächstes.
+
Sie müssen den Schalter für *Advance settings* für diese Option umlegen. Wenn Sie die applikationskonsistente Option aktivieren, müssen Sie Anmeldeinformationen in Form von Benutzername und Passwort angeben.



image:dr-plan-resource-mapping-vm-boot-order.png["Screenshot, der den Abschnitt „Virtuelle Maschine“ der Ressourcenzuordnungen zeigt"]
...
Die von dem Datenbankserver verwalteten Daten können sich auf demselben Datenspeicher wie die VM befinden, die den Datenbankserver hostet, oder auf einem anderen Datenspeicher. Die folgende Tabelle zeigt die unterstützten Konfigurationen für applikationskonsistente Snapshots in NetApp Disaster Recovery:
...
* Für Windows-VMs nutzt der Service den Microsoft Volume Shadow Copy Service (VSS), um die Koordination mit beiden Typen von Datenbankservern sicherzustellen.
...
TIP: Überprüfung der <<Überlegungen zu plattformverwalteten Backups>> vor der Auswahl dieser Optionen.

* *Plattformverwaltete Backups und Aufbewahrungspläne verwenden*: Wenn eine externe Snapshot-Verwaltungslösung verwendet wird, sollte dieses Kontrollkästchen aktiviert werden. Disaster Recovery unterstützt die Verwendung externer Snapshot-Verwaltungslösungen wie den nativen ONTAP SnapMirror Richtlinienplaner oder Integrationen von Drittanbietern. Wenn für jeden Datenspeicher (Volume) im Replikationsplan bereits eine SnapMirror Beziehung besteht, die anderweitig verwaltet wird, können diese Snapshots als Wiederherstellungspunkte in NetApp Disaster Recovery genutzt werden.
+
Wenn diese Option ausgewählt ist, konfiguriert NetApp Disaster Recovery keinen Backup-Zeitplan. Es ist jedoch weiterhin erforderlich, einen Aufbewahrungszeitplan zu konfigurieren, da für Test-, Failover- und Failback-Vorgänge möglicherweise Snapshots erstellt werden.
...
* *Startzeitpunkt für Backups und Aufbewahrung*: Geben Sie das Datum und die Uhrzeit ein, zu der Backups und Aufbewahrung beginnen sollen.
* *Backups und Aufbewahrungshäufigkeit*: Geben Sie das Zeitintervall in Stunden und Minuten ein. Wenn Sie beispielsweise 1 Stunde eingeben, erstellt der Service jede Stunde einen Snapshot.
* *Aufbewahrungsanzahl für alle Datenspeicher*: Geben Sie die Anzahl der Snapshots ein, die Sie aufbewahren möchten.
+
...
* *Quell- und Zieldatenspeicher*: Sind mehrere (verzweigte) SnapMirror Beziehungen vorhanden, kann das gewünschte Ziel ausgewählt werden. Wenn für ein Volume bereits eine SnapMirror Beziehung besteht, werden die entsprechenden Quell- und Zieldatenspeicher angezeigt. Wenn für ein Volume keine SnapMirror Beziehung besteht, kann jetzt eine erstellt werden, indem ein Ziel-Cluster, eine Ziel-SVM ausgewählt und ein Volume-Name angegeben wird. Der Dienst erstellt das Volume und die SnapMirror Beziehung.
+
...
** Während des Failovers können Sie einen beliebigen Snapshot auswählen. Wenn Sie den neuesten Snapshot auswählen, erstellt der Service eine bedarfsgesteuerte Sicherung, aktualisiert das Ziel und verwendet diesen Snapshot für das Failover.
...
== Einen Replikationsplan ändern

Sie können einen Replikationsplan-Zeitplan ändern, um die Compliance zu testen und sicherzustellen, dass Failover-Tests erfolgreich abgeschlossen werden.
